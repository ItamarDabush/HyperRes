...
def main():
    ...
    # Create model
    model = HyperRes(meta_blocks=args.meta_blocks, sigmas=args.lvls, device=args.device, bn=args.bn, bias=args.bias,
    ...
    if list(new_weights.keys())[0].startswith('module.'):
        mod_key_wieghts = {}
        for k, v in new_weights.items():
            n_k = k[len("module."):]
            mod_key_wieghts[n_k] = v
        new_weights = mod_key_wieghts
    ...
    psnr_avg = evaluate(val_loader, model)
...
def evaluate(val_loader, model):
    ...
    return {s: t.avg for s, t in zip(args.lvls, sig_loss)}
