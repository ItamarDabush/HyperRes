...
from utils.DataUtils.CommonTools import calculate_psnr, weights_init_kaiming, postProcessForStats
...
def main():
    ...
    print("=> Assembling model '{}'".format(' Multiple Objectives Network (MoNet)'))
    model = HyperRes(meta_blocks=args.meta_blocks, level=args.lvls, device=args.device,
                     bias=args.bias, gray=args.y_channel, norm_factor=args.norm_f)
...
def train(train_loader, model, criterion, optimizer):
    ...
    sig_frac = np.array([1 / len(args.lvls) for _ in args.lvls])
    st = time.time()
    for i, data in enumerate(train_loader):
        ...
        curr_step += train_loader.batch_size // 16
...
def evaluate(val_loader, val_loader_len, model, criterion):
